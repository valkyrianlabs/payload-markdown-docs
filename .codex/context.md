...
- Current release tooling state: root `VERSION` is now the canonical release version. `python -m tools.release check`, `sync`, `set-version`, `set-release`, and `bump` enforce/sync `VERSION`, root `package.json`, root `meson.build`, `debian/changelog`, and `homebrew/Formula/pmdocs.rb`. The current synced version is `0.16.1`; Debian is `0.16.1-1`, and the Homebrew formula URL targets tag `v0.16.1` with a placeholder archive SHA until release artifact checks fill it. Changelog, Debian build/validation, Nexus publication, and broader packaging code still retain Vaulthalla assumptions and must stay offline-by-default until adapted. Do not run live AI/provider or publication commands from routine local tests; keep provider and Nexus behavior mocked or disabled unless explicitly validating a protected release path.
