...
- `src/sync/` contains internal manifest/path/frontmatter/hash/validation/planning utilities and unit tests. These are server and native CLI implementation contracts, not root exports.
- There is no supported Node CLI under `src/cli/`; docs publishing uses native `pmdocs`.
- `skills/payload-markdown-docs/codex/` and `skills/payload-markdown-docs/claude/` contain bundled agent skill artifacts served as sync assets and copied by native `pmdocs install skill` into `.agents/skills/payload-markdown-docs/` or `.claude/skills/payload-markdown-docs/`.
...
- `cli/` contains the native C++ `pmdocs` implementation, Meson build, and doctest suite.
...
### 1. Maintain Native Local Commands

Status: implemented and verified for the current offline local-command scope.

Keep `pmdocs validate`, `pmdocs manifest`, and `pmdocs plan` aligned with the
server manifest contract.

Scope:

- Support native CLI flags and defaults: positional `[docs-root]`, `--docs`,
...
- Keep native tests covering `validate`, `manifest`, and `plan`.
...
- Implemented native `keygen` flags: `--format <pem|base64>`, `--out`, and
  `--force`.
...
- Document that npm installs the Payload plugin/runtime only, while `pmdocs` is
  installed through Debian/APT or Homebrew for operator commands.
- Remove or archive stale phase language once Phase 3 is complete.
- Treat failing native CLI tests, Debian package smoke checks, or Homebrew
  formula smoke checks as release blockers.

Done when:

- v1 docs describe exactly what ships.
- CI blocks regressions in native CLI, plugin package, packaging, and public API drift.
...
- Native `pmdocs install skill` installs local Codex or Claude agent guidance. Installers write Markdown files only; they must not fetch remote docs, run package managers, or mutate Payload.
