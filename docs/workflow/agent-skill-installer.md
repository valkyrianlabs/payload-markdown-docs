...
coverage or the sitemap helper `additionalRoutes` option. Pair that with
top-level `/llms.txt` and `/llms-full.txt` routes so crawlers can discover the
static AI entrypoints while agents can consume the native skill files directly.
